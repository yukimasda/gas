...
function fetchHooksFromGitHub() {
  ...
  sheet.clear();
  sheet.appendRow(['ファイル名', 'クラス名', 'フック名', 'コールバック関数名', '種別', '行番号', 'コメント']);
  ...
  try {
    ...
    files.forEach(file => {
      const content = fetchFileContent(owner, repoName, file.path);
      const lines = content.split('\n');
      let currentClass = ''; // 現在のクラスを追跡
      ...
      lines.forEach((line, index) => {
        ...
        if (line.includes('add_action(') || line.includes('add_filter(')) {
          ...
          rowBuffer.push([file.path, currentClass, hookName, callback, type, lineLink, comment]);
          totalHooks++;
